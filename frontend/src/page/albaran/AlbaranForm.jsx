...
import { albaranApi } from '../../axios/albaranApi'; // API para albaranes
import { clienteApi } from '../../axios/clienteApi'; // API para clientes
import { lineaAlbaranApi } from '../../axios/lineaAlbaranApi'; // API para líneas de albarán
...
const AlbaranForm = () => {
    ...
    // Cargar clientes y albarán si hay un ID
    useEffect(() => {
        ...
        const fetchAlbaran = async () => {
            if (id) {
                try {
                    const albaran = await albaranApi.getById(id);

                    if (albaran) {
                        setTitulo(albaran.titulo);
                        setCondiciones(albaran.condiciones);
                        setEstado(albaran.estado);
                        setIdCliente(albaran.idCliente);

                        // Cargar líneas de albarán
                        const lineasAlbaran = await lineaAlbaranApi.getAll();
                        const lineasFiltradas = lineasAlbaran.filter((linea) => linea.idAlbaran === parseInt(id));
                        setLineas(lineasFiltradas || []);
                    } else {
                        console.error(`Albarán con ID ${id} no encontrado.`);
                    }
                } catch (error) {
                    console.error('Error al cargar el albarán:', error.message);
                }
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let savedAlbaran;
            if (id) {
                savedAlbaran = await albaranApi.update(id, albaranData);
            } else {
                savedAlbaran = await albaranApi.create(albaranData);
            }

            // Guardar las líneas de albarán
            await Promise.all(
                lineas.map(async (linea) => {
                    if (linea.id) {
                        // Actualizar línea existente
                        return await lineaAlbaranApi.update(linea.id, {
                            ...linea,
                            idAlbaran: savedAlbaran.id,
                        });
                    } else {
                        // Crear nueva línea
                        return await lineaAlbaranApi.create({
                            ...linea,
                            idAlbaran: savedAlbaran.id,
                        });
                    }
            ...
            await Promise.all(
                lineasEliminadas.map(async (linea) => {
                    return await lineaAlbaranApi.delete(linea.id);
                })
            );
            ...
            console.error('Error al guardar el albarán:', error.message);
        }
    };
    ...
    return (
        <div className="bg-white text-black p-8 max-w-7xl mx-auto">
            ...
            <h2 className="text-3xl font-bold mb-6">{id ? 'Editar' : 'Nuevo'} Albarán</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="my-8">
                    <h3 className="text-xl font-semibold mb-4">Líneas de albarán</h3>
