...
import { presupuestoApi } from '../../axios/presupuestoApi'; // API para presupuestos
import { clienteApi } from '../../axios/clienteApi'; // API para clientes
import { lineaPresupuestoApi } from '../../axios/lineaPresupuestoApi'; // API para líneas de presupuesto
...
const PresupuestoForm = () => {
    ...
    // Cargar clientes y presupuesto si hay un ID
    useEffect(() => {
        ...
        const fetchPresupuesto = async () => {
            if (id) {
                try {
                    // Obtener todos los presupuestos y filtrar por el ID
                    ...
                    if (presupuesto) {
                        ...
                        setIdCliente(presupuesto.idCliente);

                        // Cargar líneas de presupuesto
                        const lineasPresupuesto = await lineaPresupuestoApi.getByPresupuestoId(presupuesto.id);
                        setLineas(lineasPresupuesto || []);
                    } else {
                        console.error(`Presupuesto con ID ${id} no encontrado.`);
                    }
                } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (id) {
                savedPresupuesto = await presupuestoApi.update(id, presupuestoData);
            } else {
                savedPresupuesto = await presupuestoApi.create(presupuestoData);
            }

            // Guardar las líneas de presupuesto
            await Promise.all(
                lineas.map(async (linea) => {
                    if (linea.id) {
                        // Actualizar línea existente
                        return await lineaPresupuestoApi.update(linea.id, {
                            ...linea,
                            idPresupuesto: savedPresupuesto.id,
                        });
                    } else {
                        // Crear nueva línea
                        return await lineaPresupuestoApi.create({
                            ...linea,
                            idPresupuesto: savedPresupuesto.id,
                        });
                    }
            ...
            await Promise.all(
                lineasEliminadas.map(async (linea) => {
                    return await lineaPresupuestoApi.delete(linea.id);
                })
            );
    ...
    return (
        <div className="bg-white text-black p-8 max-w-7xl mx-auto">
            ...
            <h2 className="text-3xl font-bold mb-6">{id ? 'Editar' : 'Nuevo'} Presupuesto</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="my-8">
                    <h3 className="text-xl font-semibold mb-4">Líneas de presupuesto</h3>
