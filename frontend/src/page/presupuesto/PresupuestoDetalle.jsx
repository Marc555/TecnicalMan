...
const PresupuestoDetalle = () => {
    const { id } = useParams(); // Capturamos el ID del presupuesto desde la URL
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const presupuestoData = await presupuestoApi.getById(id);
                setPresupuesto(presupuestoData);
                console.log("Presupuesto cargado:", presupuestoData);

                // Cargar datos del cliente
                if (presupuestoData.idCliente) {
                    const clienteData = await clienteApi.getById(presupuestoData.idCliente);
                    setCliente(clienteData);
                    console.log("Cliente cargado:", clienteData);
                }

                // Cargar líneas de presupuesto
                const lineasData = await lineaPresupuestoApi.getByPresupuestoId(parseInt(id));
                setLineas(lineasData);
                console.log("Líneas cargadas:", lineasData);
            } catch (err) {
    ...
    if (loading) return <div className="p-4 text-gray-800">Cargando presupuesto...</div>;
    ...
    return (
        <div className="bg-white p-8 mx-auto border border-gray-300 shadow-lg">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    onClick={() => navigate(`/presupuestos/editar/${id}`)}
            ...
            <header className="mb-8">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-600">Número: {presupuesto?.id}</p>
                        <p className="text-sm text-gray-600">Título: {presupuesto?.titulo}</p>
                        {presupuesto?.fechaEmitida && (
                            <p className="text-sm text-gray-600">
                                Fecha de emisión: {new Date(presupuesto.fechaEmitida).toLocaleDateString()}
                            </p>
                        )}
                        {presupuesto?.fechaValidez && (
                            <p className="text-sm text-gray-600">
                                Validez hasta: {new Date(presupuesto.fechaValidez).toLocaleDateString()}
                            </p>
                        )}
                    </div>
            ...
            <section className="mb-8">
                <h3 className="text-lg font-bold text-gray-800">Detalles del Presupuesto</h3>
            ...
            <section className="mt-8">
                ...
                <p className="text-sm text-gray-600">{presupuesto?.condiciones || "No hay condiciones específicas."}</p>
            </section>

            {/* Pie de página */}
            <footer className="mt-8 text-center text-xs text-gray-500">
                <p>Este documento es un presupuesto oficial emitido por Mi Empresa S.L.</p>
                <p>Todos los derechos reservados. Este documento no constituye una factura.</p>
            </footer>
        </div>
    );
};
