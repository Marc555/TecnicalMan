...
public class PdfFacturaService {
    ...
    public byte[] generarPdfFactura(Long idFactura) throws Exception {
        ...
        if (factura.getFechaEmitida() == null) {
            factura.setFechaEmitida(Timestamp.valueOf(now));
        }
        if (factura.getFechaValidez() == null) {
            factura.setFechaValidez(Timestamp.valueOf(now.plus(1, ChronoUnit.MONTHS)));
        }

        facturaService.update(factura.getId(), factura);

        Optional<Cliente> clienteOpt = clienteService.findById(factura.getIdCliente());
        if (clienteOpt.isEmpty())
            throw new IllegalArgumentException("Cliente no encontrado");

        Cliente cliente = clienteOpt.get();

        List<LineaFactura> lineas = lineaFacturaService.findAll().stream()
                .filter(l -> l.getIdFactura().equals(idFactura))
                .toList();
        ...
        // Factura info (número, título)
        String idFormatted = String.format("FAC%04d", factura.getId());
        Cell cellFactura = new Cell()
                .add(new Paragraph("FACTURA " + idFormatted)
        ...
        infoTable.addCell(celdaLabelDato("FECHA DE EMISION", sdf.format(factura.getFechaEmitida())));
        infoTable.addCell(celdaLabelDato("VENCIMIENTO", sdf.format(factura.getFechaValidez())));
        ...
        doc.add(new Paragraph("CONDICIONES")
                ...
                .setMarginTop(6)
                .setMarginBottom(3));
        doc.add(new Paragraph(factura.getCondiciones() != null ? factura.getCondiciones() : "-")
                .setFontSize(9)
                .setMarginBottom(10));
        ...
        doc.add(new Paragraph(
                "Esta factura tiene una validez de 30 días. Para aceptar la factura, notifíquelo por email o teléfono. La aceptación implica la aceptación de las condiciones generales de venta.")
                .setFontSize(7)
